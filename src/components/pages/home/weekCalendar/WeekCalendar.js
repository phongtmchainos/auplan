  ...
  createTable = (numberWeek) => {
    ...
    for (let i = 0; i < numberWeek; i++) {
      ...
      <table key={i}>
        ...
        <tbody>
        <tr>
          <td>{ moment().add(i * 7, 'days').format('MM') }月</td>
          <td>
            <span className={i === 0 ? 'today' : ''} >{ moment().add(i * 7, 'days').format('DD') }</span>
          </td>
          <td>
            <span className={ moment().add(1, 'days').format('dd') }>{ moment().add(i * 7 + 1, 'days').format('DD') }</span>
          </td>
          <td>
            <span className={ moment().add(2, 'days').format('dd') }>{ moment().add(i * 7 + 2, 'days').format('DD') }</span>
          </td>
          <td>
            <span className={ moment().add(3, 'days').format('dd') }>{ moment().add(i * 7 + 3, 'days').format('DD') }</span>
          </td>
          <td>
            <span className={ moment().add(4, 'days').format('dd') }>{ moment().add(i * 7 + 4, 'days').format('DD') }</span>
          </td>
          <td>
            <span className={ moment().add(5, 'days').format('dd') }>{ moment().add(i * 7 + 5, 'days').format('DD') }</span>
          </td>
          <td>
            <span className={ moment().add(6, 'days').format('dd') }>{ moment().add(i * 7 + 6, 'days').format('DD') }</span>
          </td>
        </tr>
        </tbody>
      </table>)
